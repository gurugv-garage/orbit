...
## Contents
- [0 · Dev environment](#0--dev-environment)
- [1 · node-rover (sim first, then hardware)](#1--node-rover-sim-first-then-hardware)
  - [1.1 Sim — locomotion stack](#11-sim--locomotion-stack)
  - [1.2 Sim — navigation perception](#12-sim--navigation-perception)
  - [1.3 Sim — manipulation](#13-sim--manipulation)
  - [1.4 Sim — agent interface](#14-sim--agent-interface)
  - [1.5 Hardware (later — sim must be solid first)](#15-hardware-later--sim-must-be-solid-first)
- [2 · node-dock (Android phone per location, optional servo body)](#2--node-dock-android-phone-per-location-optional-servo-body)
  - [2.1 Single-node MVP (phone at one location → plat)](#21-single-node-mvp-phone-at-one-location--plat)
  - [2.2 Multi-node (2+ phones)](#22-multi-node-2-phones)
  - [2.3 Hardening (per `PLAN.md` Section 4 risks)](#23-hardening-per-planmd-section-4-risks)
  - [2.5 Self-update (OTA) — see [ota.md](ota.md)](#25-self-update-ota--see-otamdotamd)
  - [2.4 Fallback path (if Android proves too painful)](#24-fallback-path-if-android-proves-too-painful)
- [3 · plat → orbit-station (laptop / home server / cloud)](#3--plat--orbit-station-laptop--home-server--cloud)
  - [3.0 orbit-station control plane](#30-orbit-station-control-plane)
  - [3.1 Audio pipeline — ▸ SUPERSEDED by the built perception/media stack](#31-audio-pipeline---superseded-by-the-built-perceptionmedia-stack)
  - [3.2 Vision pipeline — ▸ SUPERSEDED by the built perception pipeline](#32-vision-pipeline---superseded-by-the-built-perception-pipeline)
  - [3.3 LLM orchestration — ▸ BUILT as the server brain](#33-llm-orchestration---built-as-the-server-brain)
  - [3.4 World model — ▸ PARTLY built (per-dock); cross-device still future](#34-world-model---partly-built-per-dock-cross-device-still-future)
  - [3.5 Cross-system bridge](#35-cross-system-bridge)
  - [3.6 Degradation modes](#36-degradation-modes)
- [4 · Behaviors (end-user-visible)](#4--behaviors-end-user-visible)
- [5 · Backlog — features parked for later](#5--backlog--features-parked-for-later)
  - [node-dock](#node-dock)
  - [node-rover](#node-rover)
  - [Shared (dock ↔ rover, first feature that requires plat)](#shared-dock--rover-first-feature-that-requires-plat)
- [6 · Open questions (from PLAN.md Section 8)](#6--open-questions-from-planmd-section-8)
  - [node-rover](#node-rover-1)
  - [node-dock](#node-dock-1)
  - [plat](#plat)
