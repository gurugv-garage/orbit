...
## Contents
- [0. Why this rides the station, not a new channel](#0-why-this-rides-the-station-not-a-new-channel)
  - [0.1 Bootstrap (cabled, once) vs. steady state (network, forever)](#01-bootstrap-cabled-once-vs-steady-state-network-forever)
  - [0.2 Where the build runs — the "build box" coupling, stated plainly](#02-where-the-build-runs--the-build-box-coupling-stated-plainly)
- [1. Wire protocol additions](#1-wire-protocol-additions)
- [2. orbit-station :: `ota` module](#2-orbit-station--ota-module)
  - [2.1 Artifact store](#21-artifact-store)
  - [2.2 REST surface (identical regardless of how bytes got there)](#22-rest-surface-identical-regardless-of-how-bytes-got-there)
  - [2.3 Build hook (the on-demand build, decoupled)](#23-build-hook-the-on-demand-build-decoupled)
  - [2.4 Builds run in a tmux session — attachable + debuggable](#24-builds-run-in-a-tmux-session--attachable--debuggable)
- [3. Versioning — build-only on the wire, metadata on the station](#3-versioning--build-only-on-the-wire-metadata-on-the-station)
  - [3.0 One number on the wire: `build`](#30-one-number-on-the-wire-build)
  - [3.1 Where `build` comes from (per target)](#31-where-build-comes-from-per-target)
  - [3.2 Station-owned build metadata (`meta.json`)](#32-station-owned-build-metadata-metajson)
  - [3.3 Negotiation: the symmetric trigger](#33-negotiation-the-symmetric-trigger)
  - [3.4 Bumping checklist (per release)](#34-bumping-checklist-per-release)
- [4. node-dock body (ESP32) — `esp_https_ota` + A/B rollback](#4-node-dock-body-esp32--esp_https_ota--ab-rollback)
  - [4.1 Partition table (one-time firmware change)](#41-partition-table-one-time-firmware-change)
  - [4.2 Trigger — one new branch in `station_link.c`](#42-trigger--one-new-branch-in-station_linkc)
  - [4.3 Rollback = the safety net](#43-rollback--the-safety-net)
  - [4.4 Build in `hello` + heartbeat](#44-build-in-hello--heartbeat)
- [5. node-dock app (Android) — silent device-owner install](#5-node-dock-app-android--silent-device-owner-install)
  - [5.1 Silent install via device-owner](#51-silent-install-via-device-owner)
  - [5.2 Update flow (Kotlin, in `:app`)](#52-update-flow-kotlin-in-app)
  - [5.3 Scheduling](#53-scheduling)
- [6. Security (don't skip — these pull executable code over the wire)](#6-security-dont-skip--these-pull-executable-code-over-the-wire)
- [7. Console — trigger + watch from the orbit-station UI](#7-console--trigger--watch-from-the-orbit-station-ui)
  - [7.1 What the operator sees — one card per target (body, app)](#71-what-the-operator-sees--one-card-per-target-body-app)
  - [7.2 What the operator can trigger (buttons → REST)](#72-what-the-operator-can-trigger-buttons--rest)
  - [7.3 How the console stays live (no polling)](#73-how-the-console-stays-live-no-polling)
  - [7.4 Build session — attach & debug from the card](#74-build-session--attach--debug-from-the-card)
- [8. Build / sequencing plan](#8-build--sequencing-plan)
