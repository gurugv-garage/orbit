...
## Contents
- [0. Implementation status (living — update as stages land)](#0-implementation-status-living--update-as-stages-land)
- [1. Target architecture](#1-target-architecture)
- [2. Wire protocol v1](#2-wire-protocol-v1)
  - [Peer identity & addressing (hello v2) — the tenancy primitive](#peer-identity--addressing-hello-v2--the-tenancy-primitive)
  - [Topics](#topics)
  - [phone → station, topic `agent`](#phone--station-topic-agent)
  - [station → phone, topic `agent` (directed)](#station--phone-topic-agent-directed)
  - [topic `bodylink` (existing — now the only motion path)](#topic-bodylink-existing--now-the-only-motion-path)
  - [phone → station, topic `client` (existing)](#phone--station-topic-client-existing)
  - [Ordering, identity, and tenancy rules](#ordering-identity-and-tenancy-rules)
  - [Cross-dock interactions (designed in now, used rarely)](#cross-dock-interactions-designed-in-now-used-rarely)
- [3. Station implementation](#3-station-implementation)
  - [3.0 Tenancy, composition & session lifecycle](#30-tenancy-composition--session-lifecycle)
  - [3.1 `modules/brain/` (new)](#31-modulesbrain-new)
  - [3.2 Motion executor (grows inside `modules/bodylink/`)](#32-motion-executor-grows-inside-modulesbodylink)
  - [3.3 Dock directory (`modules/docks/` grows; bodyAddr brokering retires)](#33-dock-directory-modulesdocks-grows-bodyaddr-brokering-retires)
- [4. Phone implementation](#4-phone-implementation)
- [5. Firmware implementation (`dock_body_v0`)](#5-firmware-implementation-dock_body_v0)
- [6. Reconnection & liveness (first-class)](#6-reconnection--liveness-first-class)
- [7. Corner-case catalogue](#7-corner-case-catalogue)
- [8. Optimizations](#8-optimizations)
- [9. Simplifications](#9-simplifications)
- [10. Build order (not a rollout)](#10-build-order-not-a-rollout)
- [11. Testing plan](#11-testing-plan)
- [12. Change list (by file)](#12-change-list-by-file)
<!-- /TOC -->
