...
- [1. Why](#1-why)
- [2. The ego](#2-the-ego)
  - [2.1 The story is self-centered; its job is to justify the identity](#21-the-story-is-self-centered-its-job-is-to-justify-the-identity)
  - [2.2 Tension, and the repertoire that resolves it](#22-tension-and-the-repertoire-that-resolves-it)
  - [2.3 Which move, and how far — the disposition (subjective, not a number)](#23-which-move-and-how-far--the-disposition-subjective-not-a-number)
  - [2.4 Write/read split (the key architectural commitment)](#24-writeread-split-the-key-architectural-commitment)
  - [2.5 There is no "traits" primitive — traits are emergent](#25-there-is-no-traits-primitive--traits-are-emergent)
  - [2.6 The structure — one coherent document (starting proposal)](#26-the-structure--one-coherent-document-starting-proposal)
- [3. Implementation](#3-implementation)
  - [3.1 The ego document](#31-the-ego-document)
  - [3.2 Introspect (the only thing that evolves the ego)](#32-introspect-the-only-thing-that-evolves-the-ego)
  - [3.3 The trace (the ego's history of becoming)](#33-the-trace-the-egos-history-of-becoming)
  - [3.4 Tension (recorded, resolved in introspection)](#34-tension-recorded-resolved-in-introspection)
  - [3.5 USE — the ego as a read surface (tools, not a driver)](#35-use--the-ego-as-a-read-surface-tools-not-a-driver)
  - [3.6 Test harness](#36-test-harness)
  - [Cross-cutting risks](#cross-cutting-risks)
- [4. The three layers, and the coherence layer](#4-the-three-layers-and-the-coherence-layer)
  - [The coherence layer (and what's actually built)](#the-coherence-layer-and-whats-actually-built)
- [5. Philosophy (supporting, not central)](#5-philosophy-supporting-not-central)
<!-- /TOC -->
...
| coherence-layer concept | built? | = today (updated 2026-07-10) |
|---|---|---|
| raw snapshot ring | ✅ | the snapshot store, now **durable** on disk (retention.ts) |
| **rolling picture** (rolling summary) | ✅ | **the summarizer** — the rolling summary + the hourly **span-summaries** (its self-compressed tail) |
| durable beliefs, curator re-based onto summaries | ✅ **superseded** | the background curator was **removed**; fact-extraction is now the summarizer's 2nd trim-pass output into the same MemoryStore — one pass, one checkpoint (perception-pipeline.md §7c) |
| the ego reads perception's **clean** output, not raw | ✅ | `reconciledPerceptionSince` — the ego reads reconciled summaries, not raw sensor lines (the fix for the "my eyes are broken" spiral) |
