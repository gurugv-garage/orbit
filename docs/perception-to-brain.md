...
>
> **Status: all decisions (1–5) built and merged to `main`, plus the always-on-mic
> shift A1.1–A1.5** (branch `always-on-stt`, merge `4637d30`). Phase 1 = internal-thought
> routing (`trigger.kind:'self'`, the pure `ThoughtRouter`, the session-state accessor,
> the `think` poke). Phase 2 = grounding (last summary + raw-since injected every turn).
> Phase 3 = `force_get_current`. Phase 4 = the unified per-dock **memory store** (sqlite,
> axes + lineage + supersede, Gemini-embedded semantic recall) + the memory tool set + 4c
> inspector. Phase 5 = the **proactive attention gate** (cheap-rules tier; cooldown/dedup/
> ego-guard) + 5c toggle. A1 made the mic always-on and moved STT + turn-construction to
> the station: A1.1 always-on mic + server STT (client recognizer deleted), AEC (TTS
> through WebRTC), A1.2 tap→addressed→server turn, A1.3 final-only transcript display,
> A1.4 voice barge-in, A1.5 always-on auto-summarization. All unit + integration +
> console-verified (Playwright + real Gemini); A1 validated on hardware. The `listening`
> signal — a stub when this doc was first written — is now the real `ConversationState`
> machine. See the `docs/findings/a1.*.md` notes for the human test recipes.
>
> **What's still open** (the two real frontiers, both unblocked by A1 but not built):
> (1) the gate's **conversation-relevance tier** — the LLM-judge "should I interject into
> ambient/overheard speech?" path; the structure + rule + test exist but `signals.relevance`
> is still always undefined (the pyramid's escalation over the cheap rules). (2) the
> **face gallery → memory-store migration** (§4.5/4.6) — the gallery still runs as its own
> JSON, parallel to `MemoryStore`. Plus the deferred memory follow-up (retention/rollup
> tiers, grounding's memory-selection). This doc pins the decisions + the seams; the
> per-decision status notes below mark what's built vs. open.
...
- [A1 — the always-on-mic shift: the build plan](#a1--the-always-on-mic-shift-the-build-plan)
  - [Where the two sides stood pre-A1 (the starting point this solved)](#where-the-two-sides-stood-pre-a1-the-starting-point-this-solved)
...
> **This section states the ORIGINAL gap this doc set out to close. It is now closed**
> (all decisions + A1 built — see the status banner up top). Kept as the framing the
> decisions answer.

The gap was: a turn only happened when the **user** speaks — dock app (mic → VAD/wake →
STT) → `turn-request {text, context.state, image?}` → the brain runs a turn, appending a
per-turn grounding line (face recognition pulled on demand + one camera frame). The rich
snapshot pipeline (continuous vision/speech/identity/emotion/bodymotion + the summarizer)
was **not wired to the brain at all**. Plugging it in is the set of decisions below
(1, 2, 2b, 2c, 3, 4, 5), then the phased build — all now done.
...
1. **Internal-thought routing (Decision 2)** — the foundation. ✅ **DONE.** As built:
   ...
   - `DockBrainSession.state(): 'idle'|'listening'|'speaking'|'thinking'` over the
     ...
     `noteSpeech`, and `listening`/`followup` delegated to `ConversationState.mode()`);
   ...
   **Update (A1 landed):** `listening` is no longer a stub — it's the real
   `ConversationState` machine (`conversation-state.ts`: `mode()`/`isListening()`),
   driven by the always-on mic's taps + VAD/utterance events. All four states have real
   signals today.
...
## A1 — the always-on-mic shift: the build plan

> Branch: `always-on-stt` (**merged to `main`, `4637d30`**). This was the build-out of
> **A1**, broken into validated phases A1.1–A1.5 — all built (see the per-phase ✅ below).
> The server-side STT (`stt-watch.ts`) was already complete; the work was (a) phone audio
> plumbing and (b) a new station path that turns live utterances into brain turns. Each
> phase shipped with unit + integration + functional tests. The plan below is kept as the
> as-built record; the "today" framing in the next subsection describes the **pre-A1**
> starting point it solved.

### Where the two sides stood pre-A1 (the starting point this solved)
...
- **Station — already built, just starved of audio.** `modules/perception/processors/stt-watch.ts`
  ...
  **endpointing** (`ENDPOINT_MS=1300`, `MIN_UTTERANCE_MS`, `MAX_UTTERANCE_MS`), transcribes
  ...
  bus event with confidence, and exposes `flushAll()`. **Pre-A1** it received no audio
  because the phone didn't publish a mic track; **A1.1 fixed that** (the phone now
  publishes the always-on AEC'd audio track, and A1.2 added the `onFinal` →
  addressed-latch → turn path). *(Later note: the sidecar's default engine is now
  **Parakeet-TDT**, not Whisper — `--engine whisper` falls back; see
  [perception-pipeline.md §3](perception-pipeline.md). Parakeet returns null confidence
  metrics, so the Whisper-confidence tagging is dormant under it.)*
- **Phone — published VIDEO-ONLY, on purpose (pre-A1).** `MediaStreamer` deliberately
  omitted a mic track: a WebRTC audio source opened a **second `VOICE_COMMUNICATION`
  capture** on the shared ADM, which **starved Android SpeechRecognizer** (STT returned
  "no match" on every utterance) and degraded VAD/wake. **A1.1 resolved this** by deleting
  the on-device recognizer (Option B′ — one ADM capture feeds both perception and the
  audio track), so `MediaStreamer.kt` now publishes the always-on mic.
- **The turn path pre-A1.** `AndroidSpeechRecognizerStt` → `PerceptionEvent.Transcript(final)`
  → tap-gated in `PerceptionPipeline`/`PerceptionWiring` → `RemoteBrain` sends a
  `turn-request`. **Post-A1** the phone no longer builds turns from its own STT — it sends
  an `addressed` tap signal + the WebRTC audio; the station constructs the turn from the
  server-transcribed utterance and the phone adopts it (`adoptAutonomousTurn`).
...
- **A1.1 — single-capture audio stream, validated healthy. ✅ DONE.** The atomic swap: delete
...
- **A1.2 — tap → addressed turn (server-side turn from a live utterance). ✅ DONE.** New station path:
  ...
  on the phone.
- **A1.3 — live transcript streaming to the dock (visualization). ✅ DONE (final-only;
  partials parked).** The phone shows the station-transcribed sentence. True word-streaming
  needs partial-decode passes or an interim signal (the local STT is utterance-final, one
  transcript per endpoint) — parked as not cheap enough; we kept final-only and moved
  *where* it's rendered. The round-trip (station transcript → phone overlay) is validated.
- **A1.4 — interrupts + barge-in (A2 folds in here). ✅ DONE.** With the always-on mic,
  voice barge-in (2c.2) is real: voice-activity during `speaking` → emit the interrupt →
  the existing **supersede** path aborts the turn (`sanitizeHistory` inserts
  `(interrupted)`). The echo-gate is retired by default (A1's AEC handles self-transcribe;
  `STT_ECHO_GATE=1` re-enables it on a weak-AEC device). Tap-to-interrupt stays as the
  always-reliable fallback. Self-tested + integration variants; a positive live test (human
  talks over TTS) is the remaining functional check.
- **A1.5 — proactive (no-tap) responses + memory exercise. ⚙ PARTIAL.** Now that the
  station hears *everything*, exercise the parts that were stubbed on the live signal:
  - the **attention gate's relevance tier** (Decision 1/5) — overheard speech the gate
    ...
    **STILL OPEN:** `signals.relevance` is wired into the gate's rule + test but never
    populated (the live transcript that unblocks it now exists; the LLM-judge that fills it
    is the remaining build — the pyramid's escalation over the cheap rules);
  - **memory under a real always-on stream** — trigger summarization and verify it
    summarizes correctly, and that `remember`/`recall` behave with continuous input.
    **Done:** auto-summarization now runs always-on (`auto-summarizer.ts`, debounced per
    dock), keeping grounding fresh without a manual `/snapshots/summarize` click, per
    [memory.md](memory.md) §0/§8.
...
- **The gate's conversation-relevance tier (the main open frontier).** A1 made the live
  transcript real, so the gate can now *see* overheard speech — but the LLM-judge that
  decides "is this worth interjecting on?" is unbuilt: `signals.relevance` stays undefined.
  Open: the judge's prompt + escalation policy (cheap rules → small LLM judge, per the
  pyramid); interjection sensitivity (false-positive cost of speaking unprompted).
- **Face gallery → `MemoryStore` migration (§4.5/4.6).** The store exists; the gallery
  still runs as its own JSON behind the `FaceToolsApi` facade. Open: wrap-behind-facade
  vs. row migration; keeping the facade stable so the brain is unaffected.
- `recall_memory` exact `type` taxonomy + `time_interval` grammar + `query` semantics.
- Memory retention tiers (how long raw vs summaries; per-dock budgets) + grounding's
  memory-selection (recency × relevance × confidence).
- Staleness budgets per thought kind (how old is "too old" to say).
- **Segmentation tuning (Decision 2b), now that the mic is always-on.** The single-
  utterance addressed bracket works; the open part is whether to widen to a coherence
  window (walk back through contiguous speech to a silence gap) and the exact boundary
  rule. The always-on-mic shift itself is **done** (A1, merged) — what remains is tuning,
  not the shift.

> **Resolved since first draft:** the always-on-mic shift (A1) is built and merged
> (`4637d30`) — the Android recognizer is gone, the phone publishes always-on AEC'd audio,
> the station owns STT + turn construction, and the `listening` signal + segmentation (2b)
> + the attention gate's *plumbing* are all real. Only the gate's relevance *judgment*
> (above) is still open.
