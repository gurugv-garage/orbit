...
>
> **In progress (branch `always-on-stt`):** the **A1 always-on-mic shift** — the build
> plan is the *"A1 — the always-on-mic shift"* section near the end of this doc (phases
> A1.1–A1.5, each validated). Key framing: the server STT (`stt-watch.ts`) is already
> built; tap is redefined from *mic-on* to *addressed*; the single-ADM mic conflict makes
> "drop the client recognizer + publish WebRTC audio" one atomic step.
