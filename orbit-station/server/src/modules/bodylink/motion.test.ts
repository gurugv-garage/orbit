...
test('runSteps: no explicit duration → paces by travel distance (big sweep slower than small nudge)', async () => {
  ...
  motion.runSteps(DOCK, [{ part: 'foot', degrees: 90 }], 'test');
  await sleep(20);
  ...
  const fresh = setup();
  fresh.motion.runSteps(DOCK, [{ part: 'foot', degrees: 9 }], 'test'); // 1600µs → 100µs travel → floors at MIN
  await sleep(20);
...
test('runSteps: explicit duration_ms is honored when above the velocity cap', async () => {
  ...
  const { motion, sent } = setup();
  motion.runSteps(DOCK, [{ part: 'foot', degrees: 30, duration_ms: 300 }], 'test');
  await sleep(20);
...
test('runSteps: an explicit duration faster than comfortable is stretched to the comfortable floor', async () => {
  ...
  const { motion, sent } = setup();
  motion.runSteps(DOCK, [{ part: 'foot', degrees: -35, duration_ms: 120 }], 'test');
  await sleep(20);
...
test('runSteps: a `snap` step opts out of the comfortable floor (bounded only by the hard cap)', async () => {
  ...
  const { motion, sent } = setup();
  motion.runSteps(DOCK, [{ part: 'foot', degrees: -35, duration_ms: 120, snap: true }], 'test');
  await sleep(20);
...
test('runSteps: a snap full-sweep is still bounded by the HARD velocity cap (never a wiggle)', async () => {
  ...
  const { motion, sent } = setup();
  motion.runSteps(DOCK, [{ part: 'foot', degrees: 90, duration_ms: 120, snap: true }], 'test');
  await sleep(20);
...
test('runSteps throws offline / on bad input (the model narrates)', () => {
  const { motion } = setup([]);
  assert.throws(() => motion.runSteps(DOCK, [{ part: 'neck', degrees: 0 }], 'test'), /offline/);
  const online = setup();
  assert.throws(() => online.motion.runSteps(DOCK, [], 'test'), /at least one step/);
  assert.throws(() => online.motion.runSteps(DOCK, [{ part: 'tail', degrees: 0 }], 'test'), /unknown part/);
  assert.throws(() => online.motion.runSteps(DOCK, [{}], 'test'), /needs joints/);
...
test('runSteps throws when a HIGHER-priority holder owns the body (honest status, not a fake "moving")', () => {
  ...
  assert.throws(
    () => motion.runSteps(DOCK, [{ part: 'foot', degrees: 90 }], 'test', 'brain-turn'),
...
test('runSteps throws when the move is zero-travel (already at the limit — "turn right again")', async () => {
  ...
  assert.match(motion.runSteps(DOCK, [{ part: 'foot', degrees: 90 }], 'test', 'brain-turn'), /moving/);
  ...
  assert.throws(() => motion.runSteps(DOCK, [{ part: 'foot', degrees: 200 }], 'test', 'brain-turn'), /already there/);
  // but a move to a DIFFERENT angle travels and is fine.
  assert.match(motion.runSteps(DOCK, [{ part: 'foot', degrees: 0 }], 'test', 'brain-turn'), /moving/);
  motion.shutdown();
});

test('runSteps: a pure wait step (no joints) is exempt from the zero-travel guard', () => {
  const { motion } = setup();
  assert.match(motion.runSteps(DOCK, [{ wait_ms: 200 }], 'test', 'brain-turn'), /pausing|moving/);
  motion.shutdown();
});
...
test('runSteps: RELATIVE moves add a delta to the live pose — "turn right again" keeps moving', async () => {
  ...
  motion.runSteps(DOCK, [{ part: 'foot', degrees: -30, relative: true }], 'test', 'brain-turn');
  await sleep(20);
...
test('runSteps: a relative move CLAMPS at the limit, and once there is a zero-travel no-op (honest)', async () => {
  ...
  assert.match(motion.runSteps(DOCK, [{ part: 'foot', degrees: -200, relative: true }], 'test', 'brain-turn'), /moving/);
  await sleep(20);
...
test('pose(): reports facing + angles for the grounding line', async () => {
  ...
  motion.runSteps(DOCK, [{ part: 'foot', degrees: -45 }], 'test', 'brain-turn'); // -45° = right
  await sleep(20);
...
test('stop() cancels a running sequence; new sequence supersedes the old', async () => {
  ...
  motion.runSteps(DOCK, [{ part: 'foot', degrees: 30, duration_ms: 200 }], 'test');
  await sleep(40);
...
test('heartbeat re-sends current targets idempotently', async () => {
  const { motion, sent } = setup();
  motion.setTargets(DOCK, { neck: 1400 }, 'test', 10);
...
test('gesture: unknown expression or offline body is a silent no-op', async () => {
  const { motion, sent } = setup();
  motion.playGesture(DOCK, 'nonexistent', {}, 'test');
  motion.playGesture(DOCK, 'happy', { happy: [{ part: 'neck', degrees: -12, duration_ms: 10 }] }, 'test');
  await sleep(30);
...
test('playGesture: choreography is an OFFSET around the current pose, not absolute', async () => {
  ...
  motion.runSteps(DOCK, [{ parts: [{ part: 'foot', degrees: -80 }, { part: 'neck', degrees: -10 }], duration_ms: 200 }], 'test');
  await sleep(450);
