...
// MIC MUTE (BUG-2): mic OFF ⇒ NOT listening. Muting closes any open window and
// blocks new ones (tap/face inert); unmuting just lifts the gate (no auto re-open).
describe('ConversationState — mic mute (mic OFF ⇒ not listening)', () => {
  it('muting while listening closes the window → idle (emits mic-muted)', () => {
    const { cs, log } = make();
    cs.tap(0);
    assert.equal(cs.mode(0), 'listening');
    cs.setMuted(true, 10);
    assert.equal(cs.mode(20), 'idle', 'mute drops listening to idle');
    assert.equal(cs.isListening(20), false);
    assert.equal(log.at(-1)?.reason, 'mic-muted', 'transition reason marks the mute');
  });

  it('a tap while muted opens NO window (stays idle)', () => {
    const { cs } = make();
    cs.setMuted(true, 0);
    cs.tap(10);
    assert.equal(cs.mode(20), 'idle', 'muted tap is inert');
    assert.equal(cs.isListening(20), false);
  });

  it('a face arrival while muted opens NO window', () => {
    const { cs } = make();
    cs.setMuted(true, 0);
    cs.faceArrival(10);
    assert.equal(cs.mode(20), 'idle', 'muted face-arrival is inert');
  });

  it('unmuting does NOT auto-reopen; a tap after unmute listens again', () => {
    const { cs } = make();
    cs.tap(0);
    cs.setMuted(true, 10);
    assert.equal(cs.mode(20), 'idle');
    cs.setMuted(false, 30);
    assert.equal(cs.mode(40), 'idle', 'unmute alone does not re-open a window');
    cs.tap(50);
    assert.equal(cs.mode(60), 'listening', 'tap after unmute opens a window again');
  });

  it('muting while already idle is a clean no-op (no transition)', () => {
    const { cs, seq } = make();
    cs.setMuted(true, 0);
    assert.equal(cs.mode(0), 'idle');
    assert.deepEqual(seq(), [], 'no spurious transition');
  });

  // REGRESSION (review Finding 1): muting DURING an active turn must not let the
  // turn's completion (tts-end) re-open a followup listening window. The WebRTC mic
  // isn't actually muted, so a re-opened window would let real speech fire a turn.
  it('mute while speaking → tts-end goes to idle, NOT followup', () => {
    const { cs } = make();
    cs.tap(0);
    cs.utteranceEnded(100, 200);   // addressed → thinking
    cs.speakStart(300);            // → speaking
    cs.setMuted(true, 400);        // user mutes mid-reply; mode is 'speaking', window not closed yet
    cs.speakEnd(500);              // reply ends
    assert.equal(cs.mode(600), 'idle', 'muted tts-end must NOT open a followup window');
    assert.equal(cs.isListening(600), false);
  });

  it('mute while thinking → reply lifecycle never re-opens listening', () => {
    const { cs } = make();
    cs.tap(0);
    cs.utteranceEnded(100, 200);   // → thinking
    cs.setMuted(true, 250);        // mute while thinking
    cs.speakStart(300);            // dock speaks its reply (its own audio, fine)
    cs.speakEnd(400);              // → must go idle, not followup
    assert.equal(cs.mode(500), 'idle');
    assert.equal(cs.isListening(500), false);
  });

  // isListening is the authoritative gate (interim caption + turn admission). Even if
  // a window were somehow open, muted ⇒ isListening false and no utterance addressed.
  it('muted: isListening false and an utterance is NOT addressed even with an open window', () => {
    const { cs } = make();
    cs.tap(0);                     // listening window open
    cs.setMuted(true, 50);         // closes it → idle, but assert the gates regardless
    assert.equal(cs.isListening(60), false);
    assert.equal(cs.utteranceEnded(70, 80, 10), false, 'no utterance addressed while muted');
  });
});
