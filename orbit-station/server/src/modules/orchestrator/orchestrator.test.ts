/**
 * Orchestrator core — the pure conducting logic (decide truth tables + reconcile start/stop).
...
import {
  faceFollowBehaviour, wakeUpBehaviour, BEHAVIOURS, initBehaviourState,
  type World, type BehaviourState, type Tunings,
} from './behaviours.js';
...
test('faceFollow: disabled → off', () => {
  const r = faceFollowBehaviour.decide({ enabled: false }, baseWorld(), initBehaviourState());
...
test('faceFollow: conversation live → off (and closes any window)', () => {
  ...
  assert.equal(faceFollowBehaviour.decide({}, baseWorld({ turnActive: true }), active).desired, 'off');
  assert.equal(faceFollowBehaviour.decide({}, baseWorld({ listening: true }), active).self.windowOpenedAt, 0);
});
...
test('faceFollow: NOT idle long enough → off', () => {
  ...
  const r = faceFollowBehaviour.decide({ activateAfterMs: 300_000 }, w, initBehaviourState());
  assert.equal(r.desired, 'off');
});
...
test('faceFollow: never any conversation → idle is infinite → opens', () => {
  const r = faceFollowBehaviour.decide({ activateAfterMs: 300_000 }, baseWorld({ lastConversationMs: 0 }), initBehaviourState());
  assert.equal(r.desired, 'running');
});

test('faceFollow: stays running until runForMs, then SLEEPS', () => {
  const cfg = { activateAfterMs: 300_000, runForMs: 900_000 };
  let st = faceFollowBehaviour.decide(cfg, baseWorld({ lastConversationMs: 0 }), initBehaviourState()).self; // opens at T0
  // 10 min in (< 15) → still running
  let r = faceFollowBehaviour.decide(cfg, baseWorld({ now: T0 + 600_000, lastConversationMs: 0 }), st);
  assert.equal(r.desired, 'running'); st = r.self;
  // 15 min + 1 → window elapsed → sleep
  r = faceFollowBehaviour.decide(cfg, baseWorld({ now: T0 + 900_001, lastConversationMs: 0 }), st);
  assert.equal(r.desired, 'off');
...
test('faceFollow: after sleeping, does NOT immediately reopen (needs a fresh idle stretch)', () => {
  ...
  const slept: BehaviourState = { desired: 'off', windowOpenedAt: 0 };
  ...
  const r = faceFollowBehaviour.decide(cfg, baseWorld({ now: T0 + 900_002, lastConversationMs: 0 }), slept);
  assert.equal(r.desired, 'running');
});
...
test('wakeUp: enabled → running; disabled → off', () => {
  assert.equal(wakeUpBehaviour.decide({ enabled: true }, baseWorld(), initBehaviourState()).desired, 'running');
  assert.equal(wakeUpBehaviour.decide({ enabled: false }, baseWorld(), initBehaviourState()).desired, 'off');
});

// ── reconcile — start/stop + inproc toggle against a fake ──────────────────────────────────

interface Fake extends Effects {
  running: Set<string>; started: string[]; stopped: string[]; inproc: Map<string, boolean>;
  transitions: Array<{ b: string; from: string; to: string }>;
}
function fake(): Fake {
  const running = new Set<string>(); const started: string[] = []; const stopped: string[] = [];
  const inproc = new Map<string, boolean>(); const transitions: Fake['transitions'] = [];
  return {
    running, started, stopped, inproc, transitions,
    ...
    setInproc: (_d, b, on) => inproc.set(b, on),
...
test('reconcile: starts faceFollow when idle, enables wakeUp', () => {
  const fx = fake(); const states = new Map<string, BehaviourState>();
  // idle "forever" (no conversation) + wakeUp enabled
  reconcile('d1', BEHAVIOURS, baseWorld({ lastConversationMs: 0 }), states, cfgFor(), fx);
  assert.ok(fx.started.includes('face-follow'), 'faceFollow task started (idle window opened)');
  assert.equal(fx.inproc.get('wakeUp'), true, 'wakeUp inproc enabled');
});

test('reconcile: stops faceFollow when a conversation starts', () => {
  const fx = fake(); const states = new Map<string, BehaviourState>();
  reconcile('d1', BEHAVIOURS, baseWorld({ lastConversationMs: 0 }), states, cfgFor(), fx); // started
  assert.ok(fx.running.has('face-follow'));
  reconcile('d1', BEHAVIOURS, baseWorld({ turnActive: true, lastConversationMs: 0 }), states, cfgFor(), fx);
  assert.ok(fx.stopped.includes('face-follow'), 'faceFollow stopped when a turn is active');
  assert.ok(!fx.running.has('face-follow'));
});

test('reconcile: disabling wakeUp turns the inproc hook off', () => {
  const fx = fake(); const states = new Map<string, BehaviourState>();
  reconcile('d1', BEHAVIOURS, baseWorld(), states, cfgFor({ wakeUp: { enabled: false } }), fx);
  assert.equal(fx.inproc.get('wakeUp'), false);
});

test('reconcile: idempotent — no double-start when already running', () => {
  const fx = fake(); const states = new Map<string, BehaviourState>();
  const w = baseWorld({ lastConversationMs: 0 });
  reconcile('d1', BEHAVIOURS, w, states, cfgFor(), fx);
  reconcile('d1', BEHAVIOURS, { ...w, now: T0 + 1000 }, states, cfgFor(), fx);
...
test('reconcile: faceFollow.enabled=false keeps it off even when idle', () => {
  const fx = fake(); const states = new Map<string, BehaviourState>();
  reconcile('d1', BEHAVIOURS, baseWorld({ lastConversationMs: 0 }), states, cfgFor({ faceFollow: { enabled: false } }), fx);
