...
- **Agent framework:** our own **`:agent-core`** loop (pure-JVM: loop + tools + sessions; vocabulary in [../../docs/brain.md](../../docs/brain.md)). `DockAgent` is a thin facade; `DockStreamFn` (in `dev/orbit/dock/llm/`) is the transport. (Koog was evaluated and dropped — this loop is lighter and Android-free.)
